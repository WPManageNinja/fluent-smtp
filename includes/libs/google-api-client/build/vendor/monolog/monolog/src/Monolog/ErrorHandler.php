 ...
 */
namespace FluentSmtpLib\Monolog;
...
class ErrorHandler
{
    ...
    /** @var LogLevel::* */
    private $fatalLevel = \FluentSmtpLib\Psr\Log\LogLevel::ALERT;
    /** @var ?string */
    ...
    public function __construct(\FluentSmtpLib\Psr\Log\LoggerInterface $logger)
    {
        $this->logger = $logger;
    }
    /**
     ...
     * @return ErrorHandler
     */
    public static function register(\FluentSmtpLib\Psr\Log\LoggerInterface $logger, $errorLevelMap = [], $exceptionLevelMap = [], $fatalLevel = null) : self
    {
    ...
    public function registerFatalHandler($level = null, int $reservedMemorySize = 20) : self
    {
        ...
        $this->fatalLevel = null === $level ? \FluentSmtpLib\Psr\Log\LogLevel::ALERT : $level;
    ...
    {
        return ['ParseError' => \FluentSmtpLib\Psr\Log\LogLevel::CRITICAL, 'Throwable' => \FluentSmtpLib\Psr\Log\LogLevel::ERROR];
    }
    /**
    ...
    {
        return [\E_ERROR => \FluentSmtpLib\Psr\Log\LogLevel::CRITICAL, \E_WARNING => \FluentSmtpLib\Psr\Log\LogLevel::WARNING, \E_PARSE => \FluentSmtpLib\Psr\Log\LogLevel::ALERT, \E_NOTICE => \FluentSmtpLib\Psr\Log\LogLevel::NOTICE, \E_CORE_ERROR => \FluentSmtpLib\Psr\Log\LogLevel::CRITICAL, \E_CORE_WARNING => \FluentSmtpLib\Psr\Log\LogLevel::WARNING, \E_COMPILE_ERROR => \FluentSmtpLib\Psr\Log\LogLevel::ALERT, \E_COMPILE_WARNING => \FluentSmtpLib\Psr\Log\LogLevel::WARNING, \E_USER_ERROR => \FluentSmtpLib\Psr\Log\LogLevel::ERROR, \E_USER_WARNING => \FluentSmtpLib\Psr\Log\LogLevel::WARNING, \E_USER_NOTICE => \FluentSmtpLib\Psr\Log\LogLevel::NOTICE, \E_STRICT => \FluentSmtpLib\Psr\Log\LogLevel::NOTICE, \E_RECOVERABLE_ERROR => \FluentSmtpLib\Psr\Log\LogLevel::ERROR, \E_DEPRECATED => \FluentSmtpLib\Psr\Log\LogLevel::NOTICE, \E_USER_DEPRECATED => \FluentSmtpLib\Psr\Log\LogLevel::NOTICE];
    }
    /**
    ...
    {
        $level = \FluentSmtpLib\Psr\Log\LogLevel::ERROR;
        ...
        $this->logger->log($level, \sprintf('Uncaught Exception %s: "%s" at %s line %s', \FluentSmtpLib\Monolog\Utils::getClass($e), $e->getMessage(), $e->getFile(), $e->getLine()), ['exception' => $e]);
    ...
    public function handleError(int $code, string $message, string $file = '', int $line = 0, ?array $context = []) : bool
    {
        ...
        if (!$this->hasFatalErrorHandler || !\in_array($code, self::$fatalErrors, \true)) {
            $level = $this->errorLevelMap[$code] ?? \FluentSmtpLib\Psr\Log\LogLevel::CRITICAL;
    ...
    public function handleFatalError() : void
    {
        ...
        if ($lastError && \in_array($lastError['type'], self::$fatalErrors, \true)) {
            ...
            if ($this->logger instanceof \FluentSmtpLib\Monolog\Logger) {
