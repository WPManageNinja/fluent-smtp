...
use DateTimeZone;
use FluentSmtpLib\Monolog\Handler\HandlerInterface;
use FluentSmtpLib\Psr\Log\LoggerInterface;
use FluentSmtpLib\Psr\Log\InvalidArgumentException;
 ...
 */
class Logger implements \FluentSmtpLib\Psr\Log\LoggerInterface, \FluentSmtpLib\Monolog\ResettableInterface
{
    /**
     ...
     */
    public function pushHandler(\FluentSmtpLib\Monolog\Handler\HandlerInterface $handler) : self
    {
     ...
     */
    public function popHandler() : \FluentSmtpLib\Monolog\Handler\HandlerInterface
    {
        if (!$this->handlers) {
     ...
     */
    public function addRecord(int $level, string $message, array $context = [], ?\FluentSmtpLib\Monolog\DateTimeImmutable $datetime = null) : bool
    {
        ...
        try {
            ...
            foreach ($this->handlers as $handler) {
                if (null === $record) {
                    ...
                    $record = ['message' => $message, 'context' => $context, 'level' => $level, 'level_name' => $levelName, 'channel' => $this->name, 'datetime' => $datetime ?? new \FluentSmtpLib\Monolog\DateTimeImmutable($this->microsecondTimestamps, $this->timezone), 'extra' => []];
                    try {
    ...
    public static function getLevelName(int $level) : string
    {
        if (!isset(static::$levels[$level])) {
            throw new \FluentSmtpLib\Psr\Log\InvalidArgumentException('Level "' . $level . '" is not defined, use one of: ' . \implode(', ', \array_keys(static::$levels)));
        }
    ...
    public static function toMonologLevel($level) : int
    {
        if (\is_string($level)) {
            ...
            throw new \FluentSmtpLib\Psr\Log\InvalidArgumentException('Level "' . $level . '" is not defined, use one of: ' . \implode(', ', \array_keys(static::$levels) + static::$levels));
        }
        if (!\is_int($level)) {
            throw new \FluentSmtpLib\Psr\Log\InvalidArgumentException('Level "' . \var_export($level, \true) . '" is not defined, use one of: ' . \implode(', ', \array_keys(static::$levels) + static::$levels));
        }
        return $level;
    }
    /**
