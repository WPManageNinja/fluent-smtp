 ...
 */
class CubeHandler extends \FluentSmtpLib\Monolog\Handler\AbstractProcessingHandler
{
     ...
     */
    public function __construct(string $url, $level = \FluentSmtpLib\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
    ...
    protected function connectUdp() : void
    {
        if (!\extension_loaded('sockets')) {
            throw new \FluentSmtpLib\Monolog\Handler\MissingExtensionException('The sockets extension is required to use udp URLs with the CubeHandler');
        }
    ...
    protected function connectHttp() : void
    {
        if (!\extension_loaded('curl')) {
            throw new \FluentSmtpLib\Monolog\Handler\MissingExtensionException('The curl extension is required to use http URLs with the CubeHandler');
        }
    ...
    protected function write(array $record) : void
    {
        ...
        if ($this->scheme === 'http') {
            $this->writeHttp(\FluentSmtpLib\Monolog\Utils::jsonEncode($data));
        } else {
            $this->writeUdp(\FluentSmtpLib\Monolog\Utils::jsonEncode($data));
        }
    }
    ...
    private function writeHttp(string $data) : void
    {
        ...
        \FluentSmtpLib\Monolog\Handler\Curl\Util::execute($this->httpConnection, 5, \false);
    }
}
