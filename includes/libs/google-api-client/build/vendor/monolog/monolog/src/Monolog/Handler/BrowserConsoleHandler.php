 ...
 */
class BrowserConsoleHandler extends \FluentSmtpLib\Monolog\Handler\AbstractProcessingHandler
{
    /** @var bool */
    ...
    {
        return new \FluentSmtpLib\Monolog\Formatter\LineFormatter('[[%channel%]]{macro: autolabel} [[%level_name%]]{font-weight: bold} %message%');
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return [\FluentSmtpLib\Monolog\Logger::DEBUG => 'debug', \FluentSmtpLib\Monolog\Logger::INFO => 'info', \FluentSmtpLib\Monolog\Logger::NOTICE => 'info', \FluentSmtpLib\Monolog\Logger::WARNING => 'warn', \FluentSmtpLib\Monolog\Logger::ERROR => 'error', \FluentSmtpLib\Monolog\Logger::CRITICAL => 'error', \FluentSmtpLib\Monolog\Logger::ALERT => 'error', \FluentSmtpLib\Monolog\Logger::EMERGENCY => 'error'][$level] ?? 'log';
    }
    /**
     * @return string[]
     */
    private static function handleStyles(string $formatted) : array
    {
        ...
        foreach (\array_reverse($matches) as $match) {
            ...
            $pos = $match[0][1];
            $format = \FluentSmtpLib\Monolog\Utils::substr($format, 0, $pos) . '%c' . $match[1][0] . '%c' . \FluentSmtpLib\Monolog\Utils::substr($format, $pos + \strlen($match[0][0]));
        }
    ...
    private static function handleCustomStyles(string $style, string $string) : string
    {
        ...
        if (null === $style) {
            ...
            throw new \RuntimeException('Failed to run preg_replace_callback: ' . $pcreErrorCode . ' / ' . \FluentSmtpLib\Monolog\Utils::pcreLastErrorMessage($pcreErrorCode));
        }
        return $style;
    }
    /**
