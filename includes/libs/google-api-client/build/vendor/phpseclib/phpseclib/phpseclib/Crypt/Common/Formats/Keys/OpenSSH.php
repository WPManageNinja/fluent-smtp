...
abstract class OpenSSH
{
    ...
    public static function load($key, $password = '')
    {
        ...
        if (\strpos($key, 'BEGIN OPENSSH PRIVATE KEY') !== \false) {
            ...
            $magic = \FluentSmtpLib\phpseclib3\Common\Functions\Strings::shift($key, 15);
            if ($magic != "openssh-key-v1\x00") {
                throw new \RuntimeException('Expected openssh-key-v1');
            }
            list($ciphername, $kdfname, $kdfoptions, $numKeys) = \FluentSmtpLib\phpseclib3\Common\Functions\Strings::unpackSSH2('sssN', $key);
            if ($numKeys != 1) {
            ...
            switch ($ciphername) {
                ...
                case 'aes256-ctr':
                    ...
                    list($salt, $rounds) = \FluentSmtpLib\phpseclib3\Common\Functions\Strings::unpackSSH2('sN', $kdfoptions);
                    $crypto = new \FluentSmtpLib\phpseclib3\Crypt\AES('ctr');
            ...
            list($publicKey, $paddedKey) = \FluentSmtpLib\phpseclib3\Common\Functions\Strings::unpackSSH2('ss', $key);
            list($type) = \FluentSmtpLib\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $publicKey);
            if (isset($crypto)) {
                $paddedKey = $crypto->decrypt($paddedKey);
            }
            list($checkint1, $checkint2) = \FluentSmtpLib\phpseclib3\Common\Functions\Strings::unpackSSH2('NN', $paddedKey);
            // any leftover bytes in $paddedKey are for padding? but they should be sequential bytes. eg. 1, 2, 3, etc.
            if ($checkint1 != $checkint2) {
                if (isset($crypto)) {
                    throw new \FluentSmtpLib\phpseclib3\Exception\BadDecryptionException('Unable to decrypt key - please verify the password you are using');
                }
        ...
        list($type) = \FluentSmtpLib\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $key);
        self::checkType($type);
    ...
    {
        list(, $checkint) = \unpack('N', \FluentSmtpLib\phpseclib3\Crypt\Random::string(4));
        $comment = isset($options['comment']) ? $options['comment'] : self::$comment;
        $paddedKey = \FluentSmtpLib\phpseclib3\Common\Functions\Strings::packSSH2('NN', $checkint, $checkint) . $privateKey . \FluentSmtpLib\phpseclib3\Common\Functions\Strings::packSSH2('s', $comment);
        ...
        */
        ...
        if (!$usesEncryption) {
            $key = \FluentSmtpLib\phpseclib3\Common\Functions\Strings::packSSH2('sssNss', 'none', 'none', '', 1, $publicKey, $paddedKey);
        } else {
            $rounds = isset($options['rounds']) ? $options['rounds'] : 16;
            $salt = \FluentSmtpLib\phpseclib3\Crypt\Random::string(16);
            $kdfoptions = \FluentSmtpLib\phpseclib3\Common\Functions\Strings::packSSH2('sN', $salt, $rounds);
            $crypto = new \FluentSmtpLib\phpseclib3\Crypt\AES('ctr');
            $crypto->setPassword($password, 'bcrypt', $salt, $rounds, 32);
            $paddedKey = $crypto->encrypt($paddedKey);
            $key = \FluentSmtpLib\phpseclib3\Common\Functions\Strings::packSSH2('sssNss', 'aes256-ctr', 'bcrypt', $kdfoptions, 1, $publicKey, $paddedKey);
        }
