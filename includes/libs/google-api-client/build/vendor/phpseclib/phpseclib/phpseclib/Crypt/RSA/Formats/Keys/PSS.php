 ...
 * @author  Jim Wigginton <terrafrost@php.net>
 */
abstract class PSS extends \FluentSmtpLib\phpseclib3\Crypt\Common\Formats\Keys\PKCS8
{
    /**
     * OID Name
     *
    ...
    public static function load($key, $password = '')
    {
        ...
        $result = $components + \FluentSmtpLib\phpseclib3\Crypt\RSA\Formats\Keys\PKCS1::load($key[$type . 'Key']);
        if (isset($key[$type . 'KeyAlgorithm']['parameters'])) {
            $decoded = \FluentSmtpLib\phpseclib3\File\ASN1::decodeBER($key[$type . 'KeyAlgorithm']['parameters']);
            if ($decoded === \false) {
                throw new \UnexpectedValueException('Unable to decode parameters');
            }
            $params = \FluentSmtpLib\phpseclib3\File\ASN1::asn1map($decoded[0], \FluentSmtpLib\phpseclib3\File\ASN1\Maps\RSASSA_PSS_params::MAP);
        } else {
            $params = [];
        }
        if (isset($params['maskGenAlgorithm']['parameters'])) {
            $decoded = \FluentSmtpLib\phpseclib3\File\ASN1::decodeBER($params['maskGenAlgorithm']['parameters']);
            if ($decoded === \false) {
                throw new \UnexpectedValueException('Unable to decode parameters');
            }
            $params['maskGenAlgorithm']['parameters'] = \FluentSmtpLib\phpseclib3\File\ASN1::asn1map($decoded[0], \FluentSmtpLib\phpseclib3\File\ASN1\Maps\HashAlgorithm::MAP);
        } else {
     ...
     * @return string
     */
    public static function savePrivateKey(\FluentSmtpLib\phpseclib3\Math\BigInteger $n, \FluentSmtpLib\phpseclib3\Math\BigInteger $e, \FluentSmtpLib\phpseclib3\Math\BigInteger $d, array $primes, array $exponents, array $coefficients, $password = '', array $options = [])
    {
        self::initialize_static_variables();
        $key = \FluentSmtpLib\phpseclib3\Crypt\RSA\Formats\Keys\PKCS1::savePrivateKey($n, $e, $d, $primes, $exponents, $coefficients);
     ...
     * @return string
     */
    public static function savePublicKey(\FluentSmtpLib\phpseclib3\Math\BigInteger $n, \FluentSmtpLib\phpseclib3\Math\BigInteger $e, array $options = [])
    {
        self::initialize_static_variables();
        $key = \FluentSmtpLib\phpseclib3\Crypt\RSA\Formats\Keys\PKCS1::savePublicKey($n, $e);
        $key = \FluentSmtpLib\phpseclib3\File\ASN1::extractBER($key);
        $params = self::savePSSParams($options);
    ...
    public static function savePSSParams(array $options)
    {
        ...
        */
        $params = ['trailerField' => new \FluentSmtpLib\phpseclib3\Math\BigInteger(1)];
        if (isset($options['hash'])) {
            $params['hashAlgorithm']['algorithm'] = 'id-' . $options['hash'];
        }
        if (isset($options['MGFHash'])) {
            $temp = ['algorithm' => 'id-' . $options['MGFHash']];
            $temp = \FluentSmtpLib\phpseclib3\File\ASN1::encodeDER($temp, \FluentSmtpLib\phpseclib3\File\ASN1\Maps\HashAlgorithm::MAP);
            $params['maskGenAlgorithm'] = ['algorithm' => 'id-mgf1', 'parameters' => new \FluentSmtpLib\phpseclib3\File\ASN1\Element($temp)];
        }
        if (isset($options['saltLength'])) {
            $params['saltLength'] = new \FluentSmtpLib\phpseclib3\Math\BigInteger($options['saltLength']);
        }
        return new \FluentSmtpLib\phpseclib3\File\ASN1\Element(\FluentSmtpLib\phpseclib3\File\ASN1::encodeDER($params, \FluentSmtpLib\phpseclib3\File\ASN1\Maps\RSASSA_PSS_params::MAP));
    }
}
