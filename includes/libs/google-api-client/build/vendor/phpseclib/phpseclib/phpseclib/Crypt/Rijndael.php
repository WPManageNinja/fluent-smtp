 ...
 * @author  Jim Wigginton <terrafrost@php.net>
 */
class Rijndael extends \FluentSmtpLib\phpseclib3\Crypt\Common\BlockCipher
{
    /**
    ...
    protected function setup()
    {
        ...
        if (\is_string($this->iv) && \strlen($this->iv) != $this->block_size) {
            throw new \FluentSmtpLib\phpseclib3\Exception\InconsistentSetupException('The IV length (' . \strlen($this->iv) . ') does not match the block size (' . $this->block_size . ')');
        }
    }
    /**
    ...
    public function encrypt($plaintext)
    {
        ...
        switch ($this->engine) {
            case self::ENGINE_LIBSODIUM:
                ...
                return \FluentSmtpLib\phpseclib3\Common\Functions\Strings::shift($this->newtag, \strlen($plaintext));
            case self::ENGINE_OPENSSL_GCM:
    ...
    public function decrypt($ciphertext)
    {
        ...
        switch ($this->engine) {
            case self::ENGINE_LIBSODIUM:
                ...
                if ($plaintext === \false) {
                    $this->oldtag = \false;
                    throw new \FluentSmtpLib\phpseclib3\Exception\BadDecryptionException('Error decrypting ciphertext with libsodium');
                }
                return $plaintext;
            case self::ENGINE_OPENSSL_GCM:
                if ($this->oldtag === \false) {
                ...
                if ($plaintext === \false) {
                    $this->oldtag = \false;
                    throw new \FluentSmtpLib\phpseclib3\Exception\BadDecryptionException('Error decrypting ciphertext with OpenSSL');
                }
                return $plaintext;
        }
