 ...
 */
namespace FluentSmtpLib\phpseclib3\Net\SFTP;

use FluentSmtpLib\phpseclib3\Crypt\Common\PrivateKey;
use FluentSmtpLib\phpseclib3\Net\SFTP;
...
class Stream
{
    ...
    {
        if (\defined('FluentSmtpLib\\NET_SFTP_STREAM_LOGGING')) {
            echo "__construct()\r\n";
        }
    }
    /**
    ...
    protected function parse_path($path)
    {
        ...
        if (\preg_match('/^{[a-z0-9]+}$/i', $host)) {
            $host = \FluentSmtpLib\phpseclib3\Net\SSH2::getConnectionByResourceId($host);
            if ($host === \false) {
                return \false;
            }
            $this->sftp = $host;
            ...
            if (isset($sftp) && $sftp instanceof \FluentSmtpLib\phpseclib3\Net\SFTP) {
                $this->sftp = $sftp;
                return $path;
            }
            ...
            if (isset($context[$scheme]['privkey']) && $context[$scheme]['privkey'] instanceof \FluentSmtpLib\phpseclib3\Crypt\Common\PrivateKey) {
            ...
            if (isset(self::$instances[$host][$port][$user][(string) $pass])) {
                ...
                $this->sftp = new \FluentSmtpLib\phpseclib3\Net\SFTP($host, $port);
    ...
    private function _stream_write($data)
    {
        ...
        $result = $this->sftp->put($this->path, $data, \FluentSmtpLib\phpseclib3\Net\SFTP::SOURCE_STRING, $this->pos);
        if (isset($this->notification) && \is_callable($this->notification)) {
            if (!$result) {
                \call_user_func($this->notification, \STREAM_NOTIFY_FAILURE, \STREAM_NOTIFY_SEVERITY_ERR, $this->sftp->getLastSFTPError(), NET_SFTP_OPEN, 0, 0);
                return 0;
