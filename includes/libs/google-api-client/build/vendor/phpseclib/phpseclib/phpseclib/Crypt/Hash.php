...
class Hash
{
    ...
    public function setHash($hash)
    {
        ...
        switch ($hash) {
            ...
            default:
                if (\preg_match('#^(shake(?:128|256))-(\\d+)$#', $hash, $matches)) {
                    ...
                    throw new \FluentSmtpLib\phpseclib3\Exception\UnsupportedAlgorithmException("{$hash} is not a supported algorithm");
                }
        }
        switch ($hash) {
            case 'md2':
        ...
        if (\in_array(\substr($hash, 0, 5), ['sha3-', 'shake', 'kecca'])) {
            ...
            if (\version_compare(\PHP_VERSION, '7.1.0') < 0 || \substr($hash, 0, 5) != 'sha3-') {
                ...
                $hash = ['FluentSmtpLib\\phpseclib3\\Crypt\\Hash', \PHP_INT_SIZE == 8 ? 'sha3_64' : 'sha3_32'];
            }
        }
            ...
            if (\version_compare(\PHP_VERSION, '7.1.0') < 0) {
                ...
                for ($i = 0; $i < 8; $i++) {
                    if (\PHP_INT_SIZE == 8) {
                        ...
                        $initial[$i] = new \FluentSmtpLib\phpseclib3\Math\BigInteger($initial[$i], 16);
                        $initial[$i]->setPrecision(64);
                    }
                }
                $this->parameters = \compact('initial');
                $hash = ['FluentSmtpLib\\phpseclib3\\Crypt\\Hash', \PHP_INT_SIZE == 8 ? 'sha512_64' : 'sha512'];
            }
        }
        if (\is_array($hash)) {
            $b = $this->blockSize >> 3;
    ...
    private function pdf()
    {
        ...
        //
        $kp = $this->kdf(0, 16);
        $c = new \FluentSmtpLib\phpseclib3\Crypt\AES('ctr');
        $c->disablePadding();
        $c->setKey($kp);
        $c->setIV($nonce);
    ...
    private static function L2Hash($k, $m)
    {
        ...
        $k64 = new \FluentSmtpLib\phpseclib3\Math\BigInteger($k64, 256);
        $k128 = \substr($k, 8) & "\x01\xff\xff\xff\x01\xff\xff\xff\x01\xff\xff\xff\x01\xff\xff\xff";
        $k128 = new \FluentSmtpLib\phpseclib3\Math\BigInteger($k128, 256);
        //
    ...
    private static function poly($wordbits, $maxwordrange, $k, $m)
    {
        ...
        //
        $y = $factory->newInteger(new \FluentSmtpLib\phpseclib3\Math\BigInteger(1));
        foreach ($m_i as $m) {
            $m = $factory->newInteger(new \FluentSmtpLib\phpseclib3\Math\BigInteger($m, 256));
    ...
    private static function L3Hash($k1, $k2, $m)
    {
        ...
        $y = $factory->newInteger(new \FluentSmtpLib\phpseclib3\Math\BigInteger());
        for ($i = 0; $i < 8; $i++) {
            $m_i = $factory->newInteger(new \FluentSmtpLib\phpseclib3\Math\BigInteger(\substr($m, 2 * $i, 2), 256));
            $k_i = $factory->newInteger(new \FluentSmtpLib\phpseclib3\Math\BigInteger(\substr($k1, 8 * $i, 8), 256));
        ...
        if ($algo == 'umac') {
            if ($this->recomputeAESKey) {
                if (!\is_string($this->nonce)) {
                    throw new \FluentSmtpLib\phpseclib3\Exception\InsufficientSetupException('No nonce has been set');
                }
                if (!\is_string($this->key)) {
                    throw new \FluentSmtpLib\phpseclib3\Exception\InsufficientSetupException('No key has been set');
                }
                ...
                if (!isset(self::$maxwordrange64)) {
                    ...
                    $prime36 = new \FluentSmtpLib\phpseclib3\Math\BigInteger("\x00\x00\x00\x0f\xff\xff\xff\xfb", 256);
                    self::$factory36 = new \FluentSmtpLib\phpseclib3\Math\PrimeField($prime36);
                    $prime64 = new \FluentSmtpLib\phpseclib3\Math\BigInteger("\xff\xff\xff\xff\xff\xff\xff\xc5", 256);
                    self::$factory64 = new \FluentSmtpLib\phpseclib3\Math\PrimeField($prime64);
                    $prime128 = new \FluentSmtpLib\phpseclib3\Math\BigInteger("\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xffa", 256);
                    self::$factory128 = new \FluentSmtpLib\phpseclib3\Math\PrimeField($prime128);
                    self::$offset64 = new \FluentSmtpLib\phpseclib3\Math\BigInteger("\x01\x00\x00\x00\x00\x00\x00\x00\x00", 256);
                    self::$offset64 = self::$factory64->newInteger(self::$offset64->subtract($prime64));
                    self::$offset128 = new \FluentSmtpLib\phpseclib3\Math\BigInteger("\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 256);
                ...
                $this->c = new \FluentSmtpLib\phpseclib3\Crypt\AES('ctr');
    ...
    private static function sha512($m, $hash)
    {
        ...
        if (!isset($k)) {
            ...
            for ($i = 0; $i < 80; $i++) {
                $k[$i] = new \FluentSmtpLib\phpseclib3\Math\BigInteger($k[$i], 16);
            }
        }
        // Pre-processing
        $length = \strlen($m);
        ...
        foreach ($chunks as $chunk) {
            $w = [];
            for ($i = 0; $i < 16; $i++) {
                $temp = new \FluentSmtpLib\phpseclib3\Math\BigInteger(\FluentSmtpLib\phpseclib3\Common\Functions\Strings::shift($chunk, 8), 256);
    ...
    private static function sha512_64($m, $hash)
    {
        ...
        foreach ($chunks as $chunk) {
            $w = [];
            for ($i = 0; $i < 16; $i++) {
                list(, $w[]) = \unpack('J', \FluentSmtpLib\phpseclib3\Common\Functions\Strings::shift($chunk, 8));
            }
            // Extend the sixteen 32-bit words into eighty 32-bit words
            for ($i = 16; $i < 80; $i++) {
