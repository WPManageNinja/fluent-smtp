...
abstract class PKCS8 extends \FluentSmtpLib\phpseclib3\Crypt\Common\Formats\Keys\PKCS8
{
    ...
    public static function load($key, $password = '')
    {
        ...
        $components = \FluentSmtpLib\phpseclib3\File\ASN1::asn1map($decoded[0], \FluentSmtpLib\phpseclib3\File\ASN1\Maps\DSAParams::MAP);
        if (!\is_array($components)) {
            throw new \RuntimeException('Unable to perform ASN1 mapping on parameters');
        }
        $decoded = \FluentSmtpLib\phpseclib3\File\ASN1::decodeBER($key[$type]);
        if (empty($decoded)) {
            throw new \RuntimeException('Unable to decode BER');
        }
        $var = $type == 'privateKey' ? 'x' : 'y';
        $components[$var] = \FluentSmtpLib\phpseclib3\File\ASN1::asn1map($decoded[0], \FluentSmtpLib\phpseclib3\File\ASN1\Maps\DSAPublicKey::MAP);
        if (!$components[$var] instanceof \FluentSmtpLib\phpseclib3\Math\BigInteger) {
    ...
    {
        $params = ['p' => $p, 'q' => $q, 'g' => $g];
        $params = \FluentSmtpLib\phpseclib3\File\ASN1::encodeDER($params, \FluentSmtpLib\phpseclib3\File\ASN1\Maps\DSAParams::MAP);
        $params = new \FluentSmtpLib\phpseclib3\File\ASN1\Element($params);
        $key = \FluentSmtpLib\phpseclib3\File\ASN1::encodeDER($x, \FluentSmtpLib\phpseclib3\File\ASN1\Maps\DSAPublicKey::MAP);
     ...
     * @return string
     */
    public static function savePublicKey(\FluentSmtpLib\phpseclib3\Math\BigInteger $p, \FluentSmtpLib\phpseclib3\Math\BigInteger $q, \FluentSmtpLib\phpseclib3\Math\BigInteger $g, \FluentSmtpLib\phpseclib3\Math\BigInteger $y, array $options = [])
    {
        $params = ['p' => $p, 'q' => $q, 'g' => $g];
        $params = \FluentSmtpLib\phpseclib3\File\ASN1::encodeDER($params, \FluentSmtpLib\phpseclib3\File\ASN1\Maps\DSAParams::MAP);
        $params = new \FluentSmtpLib\phpseclib3\File\ASN1\Element($params);
