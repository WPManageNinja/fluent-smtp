...
use FluentSmtpLib\phpseclib3\Crypt\DES;
use FluentSmtpLib\phpseclib3\Crypt\Random;
use FluentSmtpLib\phpseclib3\Crypt\RC2;
 ...
 * @author  Jim Wigginton <terrafrost@php.net>
 */
abstract class PKCS8 extends \FluentSmtpLib\phpseclib3\Crypt\Common\Formats\Keys\PKCS
{
    /**
    ...
    private static function getPBES1EncryptionObject($algo)
    {
        ...
        switch ($algo) {
            case 'DES':
                $cipher = new \FluentSmtpLib\phpseclib3\Crypt\DES('cbc');
                break;
            case 'RC2':
                $cipher = new \FluentSmtpLib\phpseclib3\Crypt\RC2('cbc');
                $cipher->setKeyLength(64);
                break;
            case '3-KeyTripleDES':
                $cipher = new \FluentSmtpLib\phpseclib3\Crypt\TripleDES('cbc');
                break;
            case '2-KeyTripleDES':
                $cipher = new \FluentSmtpLib\phpseclib3\Crypt\TripleDES('cbc');
            ...
            case '40BitRC2':
                ...
                break;
            case '128BitRC4':
                $cipher = new \FluentSmtpLib\phpseclib3\Crypt\RC4();
                $cipher->setKeyLength(128);
                break;
            case '40BitRC4':
                $cipher = new \FluentSmtpLib\phpseclib3\Crypt\RC4();
                ...
                break;
            default:
                throw new \FluentSmtpLib\phpseclib3\Exception\UnsupportedAlgorithmException("{$algo} is not a supported algorithm");
        }
        return $cipher;
    }
    /**
    ...
    private static function getPBES2EncryptionObject($algo)
    {
        switch ($algo) {
            ...
            case 'rc2CBC':
                ...
                break;
            case 'rc5-CBC-PAD':
                throw new \FluentSmtpLib\phpseclib3\Exception\UnsupportedAlgorithmException('rc5-CBC-PAD is not supported for PBES2 PKCS#8 keys');
            case 'aes128-CBC-PAD':
            case 'aes192-CBC-PAD':
            case 'aes256-CBC-PAD':
                $cipher = new \FluentSmtpLib\phpseclib3\Crypt\AES('cbc');
                $cipher->setKeyLength(\substr($algo, 3, 3));
                break;
            default:
                throw new \FluentSmtpLib\phpseclib3\Exception\UnsupportedAlgorithmException("{$algo} is not supported");
        }
        return $cipher;
    }
    /**
    ...
    private static function initialize_static_variables()
    {
        if (!isset(static::$childOIDsLoaded)) {
            throw new \FluentSmtpLib\phpseclib3\Exception\InsufficientSetupException('This class should not be called directly');
        }
        if (!static::$childOIDsLoaded) {
            \FluentSmtpLib\phpseclib3\File\ASN1::loadOIDs(\is_array(static::OID_NAME) ? \array_combine(static::OID_NAME, static::OID_VALUE) : [static::OID_NAME => static::OID_VALUE]);
    ...
    protected static function load($key, $password = '')
    {
        ...
        $meta = [];
        $decrypted = \FluentSmtpLib\phpseclib3\File\ASN1::asn1map($decoded[0], \FluentSmtpLib\phpseclib3\File\ASN1\Maps\EncryptedPrivateKeyInfo::MAP);
        if (\strlen($password) && \is_array($decrypted)) {
            ...
            switch ($algorithm) {
                ...
                case 'pbeWithSHAAnd40BitRC4':
                    ...
                    $temp = \FluentSmtpLib\phpseclib3\File\ASN1::decodeBER($decrypted['encryptionAlgorithm']['parameters']);
                    if (!$temp) {
                        throw new \RuntimeException('Unable to decode BER');
                    }
                    \extract(\FluentSmtpLib\phpseclib3\File\ASN1::asn1map($temp[0], \FluentSmtpLib\phpseclib3\File\ASN1\Maps\PBEParameter::MAP));
                ...
                case 'id-PBES2':
                    ...
                    $temp = \FluentSmtpLib\phpseclib3\File\ASN1::asn1map($temp[0], \FluentSmtpLib\phpseclib3\File\ASN1\Maps\PBES2params::MAP);
                    \extract($temp);
                    if (!$cipher instanceof \FluentSmtpLib\phpseclib3\Crypt\RC2) {
                        $cipher->setIV($encryptionScheme['parameters']['octetString']);
                    } else {
                        $temp = \FluentSmtpLib\phpseclib3\File\ASN1::decodeBER($encryptionScheme['parameters']);
                        if (!$temp) {
                            throw new \RuntimeException('Unable to decode BER');
                        }
                        \extract(\FluentSmtpLib\phpseclib3\File\ASN1::asn1map($temp[0], \FluentSmtpLib\phpseclib3\File\ASN1\Maps\RC2CBCParameter::MAP));
                    ...
                    switch ($keyDerivationFunc['algorithm']) {
                        case 'id-PBKDF2':
                            $temp = \FluentSmtpLib\phpseclib3\File\ASN1::decodeBER($keyDerivationFunc['parameters']);
                            if (!$temp) {
                                throw new \RuntimeException('Unable to decode BER');
                            }
                            $prf = ['algorithm' => 'id-hmacWithSHA1'];
                            $params = \FluentSmtpLib\phpseclib3\File\ASN1::asn1map($temp[0], \FluentSmtpLib\phpseclib3\File\ASN1\Maps\PBKDF2params::MAP);
                            \extract($params);
                            ...
                            break;
                        default:
                            throw new \FluentSmtpLib\phpseclib3\Exception\UnsupportedAlgorithmException('Only PBKDF2 is supported for PBES2 PKCS#8 keys');
                    }
                    break;
                case 'id-PBMAC1':
                    ...
                    throw new \FluentSmtpLib\phpseclib3\Exception\UnsupportedAlgorithmException('Only PBES1 and PBES2 PKCS#8 keys are supported.');
            }
        }
        $private = \FluentSmtpLib\phpseclib3\File\ASN1::asn1map($decoded[0], \FluentSmtpLib\phpseclib3\File\ASN1\Maps\OneAsymmetricKey::MAP);
        if (\is_array($private)) {
            if ($isPublic) {
                throw new \UnexpectedValueException('Human readable string claims public key but DER encoded string claims private key');
            }
            if (isset($private['privateKeyAlgorithm']['parameters']) && !$private['privateKeyAlgorithm']['parameters'] instanceof \FluentSmtpLib\phpseclib3\File\ASN1\Element && isset($decoded[0]['content'][1]['content'][1])) {
                $temp = $decoded[0]['content'][1]['content'][1];
                $private['privateKeyAlgorithm']['parameters'] = new \FluentSmtpLib\phpseclib3\File\ASN1\Element(\substr($key, $temp['start'], $temp['length']));
            }
            if (\is_array(static::OID_NAME)) {
                if (!\in_array($private['privateKeyAlgorithm']['algorithm'], static::OID_NAME)) {
                    throw new \FluentSmtpLib\phpseclib3\Exception\UnsupportedAlgorithmException($private['privateKeyAlgorithm']['algorithm'] . ' is not a supported key type');
                }
            } else {
                if ($private['privateKeyAlgorithm']['algorithm'] != static::OID_NAME) {
                    throw new \FluentSmtpLib\phpseclib3\Exception\UnsupportedAlgorithmException('Only ' . static::OID_NAME . ' keys are supported; this is a ' . $private['privateKeyAlgorithm']['algorithm'] . ' key');
                }
            }
        ...
        $public = \FluentSmtpLib\phpseclib3\File\ASN1::asn1map($decoded[0], \FluentSmtpLib\phpseclib3\File\ASN1\Maps\PublicKeyInfo::MAP);
        if (\is_array($public)) {
            if ($isPrivate) {
            ...
            if (\is_array(static::OID_NAME)) {
                if (!\in_array($public['publicKeyAlgorithm']['algorithm'], static::OID_NAME)) {
                    throw new \FluentSmtpLib\phpseclib3\Exception\UnsupportedAlgorithmException($public['publicKeyAlgorithm']['algorithm'] . ' is not a supported key type');
                }
            } else {
                if ($public['publicKeyAlgorithm']['algorithm'] != static::OID_NAME) {
                    throw new \FluentSmtpLib\phpseclib3\Exception\UnsupportedAlgorithmException('Only ' . static::OID_NAME . ' keys are supported; this is a ' . $public['publicKeyAlgorithm']['algorithm'] . ' key');
                }
            }
            if (isset($public['publicKeyAlgorithm']['parameters']) && !$public['publicKeyAlgorithm']['parameters'] instanceof \FluentSmtpLib\phpseclib3\File\ASN1\Element && isset($decoded[0]['content'][0]['content'][1])) {
                $temp = $decoded[0]['content'][0]['content'][1];
                $public['publicKeyAlgorithm']['parameters'] = new \FluentSmtpLib\phpseclib3\File\ASN1\Element(\substr($key, $temp['start'], $temp['length']));
            }
    ...
    protected static function wrapPrivateKey($key, $attr, $params, $password, $oid = null, $publicKey = '', array $options = [])
    {
        ...
        $key = \FluentSmtpLib\phpseclib3\File\ASN1::encodeDER($key, \FluentSmtpLib\phpseclib3\File\ASN1\Maps\OneAsymmetricKey::MAP);
        if (!empty($password) && \is_string($password)) {
            ...
            if ($encryptionAlgorithm == 'id-PBES2') {
                ...
                $kdf = 'pbkdf2';
                $iv = \FluentSmtpLib\phpseclib3\Crypt\Random::string($crypto->getBlockLength() >> 3);
                $PBKDF2params = ['salt' => $salt, 'iterationCount' => $iterationCount, 'prf' => ['algorithm' => $prf, 'parameters' => null]];
                $PBKDF2params = \FluentSmtpLib\phpseclib3\File\ASN1::encodeDER($PBKDF2params, \FluentSmtpLib\phpseclib3\File\ASN1\Maps\PBKDF2params::MAP);
                if (!$crypto instanceof \FluentSmtpLib\phpseclib3\Crypt\RC2) {
                    $params = ['octetString' => $iv];
                } else {
                    $params = ['rc2ParametersVersion' => 58, 'iv' => $iv];
                    $params = \FluentSmtpLib\phpseclib3\File\ASN1::encodeDER($params, \FluentSmtpLib\phpseclib3\File\ASN1\Maps\RC2CBCParameter::MAP);
                    $params = new \FluentSmtpLib\phpseclib3\File\ASN1\Element($params);
                }
                $params = ['keyDerivationFunc' => ['algorithm' => 'id-PBKDF2', 'parameters' => new \FluentSmtpLib\phpseclib3\File\ASN1\Element($PBKDF2params)], 'encryptionScheme' => ['algorithm' => $encryptionScheme, 'parameters' => $params]];
                ...
                $params = \FluentSmtpLib\phpseclib3\File\ASN1::encodeDER($params, \FluentSmtpLib\phpseclib3\File\ASN1\Maps\PBEParameter::MAP);
            }
            $crypto->setPassword($password, $kdf, $hash, $salt, $iterationCount);
            $key = $crypto->encrypt($key);
            $key = ['encryptionAlgorithm' => ['algorithm' => $encryptionAlgorithm, 'parameters' => new \FluentSmtpLib\phpseclib3\File\ASN1\Element($params)], 'encryptedData' => $key];
            $key = \FluentSmtpLib\phpseclib3\File\ASN1::encodeDER($key, \FluentSmtpLib\phpseclib3\File\ASN1\Maps\EncryptedPrivateKeyInfo::MAP);
            if (isset($options['binary']) ? $options['binary'] : self::$binary) {
                return $key;
            }
            return "-----BEGIN ENCRYPTED PRIVATE KEY-----\r\n" . \chunk_split(\FluentSmtpLib\phpseclib3\Common\Functions\Strings::base64_encode($key), 64) . "-----END ENCRYPTED PRIVATE KEY-----";
        }
        if (isset($options['binary']) ? $options['binary'] : self::$binary) {
            return $key;
        }
        return "-----BEGIN PRIVATE KEY-----\r\n" . \chunk_split(\FluentSmtpLib\phpseclib3\Common\Functions\Strings::base64_encode($key), 64) . "-----END PRIVATE KEY-----";
    }
    /**
    ...
    protected static function wrapPublicKey($key, $params, $oid = null, array $options = [])
    {
        ...
        $key = \FluentSmtpLib\phpseclib3\File\ASN1::encodeDER($key, \FluentSmtpLib\phpseclib3\File\ASN1\Maps\PublicKeyInfo::MAP);
        if (isset($options['binary']) ? $options['binary'] : self::$binary) {
            return $key;
        }
        return "-----BEGIN PUBLIC KEY-----\r\n" . \chunk_split(\FluentSmtpLib\phpseclib3\Common\Functions\Strings::base64_encode($key), 64) . "-----END PUBLIC KEY-----";
    }
    /**
    ...
    private static function preParse(&$key)
    {
        ...
        if (self::$format != self::MODE_DER) {
            $decoded = \FluentSmtpLib\phpseclib3\File\ASN1::extractBER($key);
            if ($decoded !== \false) {
                $key = $decoded;
    ...
    public static function extractEncryptionAlgorithm($key)
    {
        ...
        $decoded = self::preParse($key);
        $r = \FluentSmtpLib\phpseclib3\File\ASN1::asn1map($decoded[0], \FluentSmtpLib\phpseclib3\File\ASN1\Maps\EncryptedPrivateKeyInfo::MAP);
        if (!\is_array($r)) {
            throw new \RuntimeException('Unable to parse using EncryptedPrivateKeyInfo map');
        }
        if ($r['encryptionAlgorithm']['algorithm'] == 'id-PBES2') {
            $decoded = \FluentSmtpLib\phpseclib3\File\ASN1::decodeBER($r['encryptionAlgorithm']['parameters']->element);
            if (!$decoded) {
                throw new \RuntimeException('Unable to decode BER');
            }
            ...
            switch ($kdf['algorithm']) {
                case 'id-PBKDF2':
                    $decoded = \FluentSmtpLib\phpseclib3\File\ASN1::decodeBER($kdf['parameters']->element);
                    if (!$decoded) {
                        throw new \RuntimeException('Unable to decode BER');
                    }
                    $kdf['parameters'] = \FluentSmtpLib\phpseclib3\File\ASN1::asn1map($decoded[0], \FluentSmtpLib\phpseclib3\File\ASN1\Maps\PBKDF2params::MAP);
            }
        }
