...
use FluentSmtpLib\phpseclib3\Crypt\DH\PrivateKey;
use FluentSmtpLib\phpseclib3\Crypt\DH\PublicKey;
 ...
 * @author  Jim Wigginton <terrafrost@php.net>
 */
abstract class DH extends \FluentSmtpLib\phpseclib3\Crypt\Common\AsymmetricKey
{
    /**
     * Algorithm Name
     *
    ...
    public static function createParameters(...$args)
    {
        ...
        $params = new \FluentSmtpLib\phpseclib3\Crypt\DH\Parameters();
        if (\count($args) == 2 && $args[0] instanceof \FluentSmtpLib\phpseclib3\Math\BigInteger && $args[1] instanceof \FluentSmtpLib\phpseclib3\Math\BigInteger) {
            //if (!$args[0]->isPrime()) {
            ...
            $params->prime = \FluentSmtpLib\phpseclib3\Math\BigInteger::randomPrime($args[0]);
            $params->base = new \FluentSmtpLib\phpseclib3\Math\BigInteger(2);
            return $params;
        ...
        $params->prime = new \FluentSmtpLib\phpseclib3\Math\BigInteger($prime, 16);
        $params->base = new \FluentSmtpLib\phpseclib3\Math\BigInteger(2);
        return $params;
    }
    /**
     ...
     * @return PrivateKey
     */
    public static function createKey(\FluentSmtpLib\phpseclib3\Crypt\DH\Parameters $params, $length = 0)
    {
        $class = new \ReflectionClass(static::class);
        if ($class->isFinal()) {
        ...
        $key = new \FluentSmtpLib\phpseclib3\Crypt\DH\PrivateKey();
        $key->prime = $params->prime;
        $key->base = $params->base;
        $key->privateKey = \FluentSmtpLib\phpseclib3\Math\BigInteger::randomRange($one, $max);
    ...
    {
        if ($private instanceof \FluentSmtpLib\phpseclib3\Crypt\DH\PrivateKey) {
            // DH\PrivateKey
            switch (\true) {
                case $public instanceof \FluentSmtpLib\phpseclib3\Crypt\DH\PublicKey:
                ...
                case \is_string($public):
                    $public = new \FluentSmtpLib\phpseclib3\Math\BigInteger($public, -256);
                // fall-through
                case $public instanceof \FluentSmtpLib\phpseclib3\Math\BigInteger:
        ...
        if ($private instanceof \FluentSmtpLib\phpseclib3\Crypt\EC\PrivateKey) {
            switch (\true) {
                case $public instanceof \FluentSmtpLib\phpseclib3\Crypt\EC\PublicKey:
    ...
    protected static function onLoad(array $components)
    {
        if (!isset($components['privateKey']) && !isset($components['publicKey'])) {
            $new = new \FluentSmtpLib\phpseclib3\Crypt\DH\Parameters();
        } else {
            $new = isset($components['privateKey']) ? new \FluentSmtpLib\phpseclib3\Crypt\DH\PrivateKey() : new \FluentSmtpLib\phpseclib3\Crypt\DH\PublicKey();
        }
    ...
    {
        throw new \FluentSmtpLib\phpseclib3\Exception\UnsupportedOperationException('DH does not use a hash algorithm');
    }
    /**
    ...
    {
        $type = \FluentSmtpLib\phpseclib3\Crypt\DH::validatePlugin('Keys', 'PKCS1', 'saveParameters');
        $key = $type::saveParameters($this->prime, $this->base);
        return \FluentSmtpLib\phpseclib3\Crypt\DH::load($key, 'PKCS1');
    }
}
