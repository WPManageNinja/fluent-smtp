...
abstract class IEEE
{
    ...
    public static function load($sig)
    {
        ...
        $r = new \FluentSmtpLib\phpseclib3\Math\BigInteger(\substr($sig, 0, $len >> 1), 256);
        $s = new \FluentSmtpLib\phpseclib3\Math\BigInteger(\substr($sig, $len >> 1), 256);
        return \compact('r', 's');
    }
    /**
     ...
     * @return string
     */
    public static function save(\FluentSmtpLib\phpseclib3\Math\BigInteger $r, \FluentSmtpLib\phpseclib3\Math\BigInteger $s, $curve, $length)
    {
        $r = $r->toBytes();
        $s = $s->toBytes();
