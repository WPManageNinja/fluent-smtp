...
abstract class XML
{
    ...
    public static function load($key, $password = '')
    {
        ...
        foreach ($keys as $key) {
            ...
            $value = new \FluentSmtpLib\phpseclib3\Math\BigInteger(\FluentSmtpLib\phpseclib3\Common\Functions\Strings::base64_decode($temp->item(0)->nodeValue), 256);
            switch ($key) {
                case 'modulus':
    ...
    public static function savePrivateKey(\FluentSmtpLib\phpseclib3\Math\BigInteger $n, \FluentSmtpLib\phpseclib3\Math\BigInteger $e, \FluentSmtpLib\phpseclib3\Math\BigInteger $d, array $primes, array $exponents, array $coefficients, $password = '')
    {
        ...
        if (!empty($password) && \is_string($password)) {
            throw new \FluentSmtpLib\phpseclib3\Exception\UnsupportedFormatException('XML private keys do not support encryption');
        }
        return "<RSAKeyPair>\r\n" . '  <Modulus>' . \FluentSmtpLib\phpseclib3\Common\Functions\Strings::base64_encode($n->toBytes()) . "</Modulus>\r\n" . '  <Exponent>' . \FluentSmtpLib\phpseclib3\Common\Functions\Strings::base64_encode($e->toBytes()) . "</Exponent>\r\n" . '  <P>' . \FluentSmtpLib\phpseclib3\Common\Functions\Strings::base64_encode($primes[1]->toBytes()) . "</P>\r\n" . '  <Q>' . \FluentSmtpLib\phpseclib3\Common\Functions\Strings::base64_encode($primes[2]->toBytes()) . "</Q>\r\n" . '  <DP>' . \FluentSmtpLib\phpseclib3\Common\Functions\Strings::base64_encode($exponents[1]->toBytes()) . "</DP>\r\n" . '  <DQ>' . \FluentSmtpLib\phpseclib3\Common\Functions\Strings::base64_encode($exponents[2]->toBytes()) . "</DQ>\r\n" . '  <InverseQ>' . \FluentSmtpLib\phpseclib3\Common\Functions\Strings::base64_encode($coefficients[2]->toBytes()) . "</InverseQ>\r\n" . '  <D>' . \FluentSmtpLib\phpseclib3\Common\Functions\Strings::base64_encode($d->toBytes()) . "</D>\r\n" . '</RSAKeyPair>';
    }
    /**
