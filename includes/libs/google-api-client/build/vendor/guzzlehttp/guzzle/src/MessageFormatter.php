 ...
 * @final
 */
class MessageFormatter implements \FluentSmtpLib\GuzzleHttp\MessageFormatterInterface
{
    /**
    ...
    public function format(\FluentSmtpLib\Psr\Http\Message\RequestInterface $request, ?\FluentSmtpLib\Psr\Http\Message\ResponseInterface $response = null, ?\Throwable $error = null) : string
    {
        ...
        return \preg_replace_callback('/{\\s*([A-Za-z_\\-\\.0-9]+)\\s*}/', function (array $matches) use($request, $response, $error, &$cache) {
            ...
            switch ($matches[1]) {
                case 'request':
                    $result = \FluentSmtpLib\GuzzleHttp\Psr7\Message::toString($request);
                    break;
                case 'response':
                    $result = $response ? \FluentSmtpLib\GuzzleHttp\Psr7\Message::toString($response) : '';
                    break;
                case 'req_headers':
                ...
                case 'req_body':
                    ...
                    break;
                case 'res_body':
                    if (!$response instanceof \FluentSmtpLib\Psr\Http\Message\ResponseInterface) {
                        $result = 'NULL';
                        break;
                    }
     ...
     */
    private function headers(\FluentSmtpLib\Psr\Http\Message\MessageInterface $message) : string
    {
        $result = '';
