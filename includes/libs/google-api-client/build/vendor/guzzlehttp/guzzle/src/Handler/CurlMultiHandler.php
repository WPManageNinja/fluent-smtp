...
use FluentSmtpLib\GuzzleHttp\Promise\Promise;
...
class CurlMultiHandler
{
    ...
    {
        $this->factory = $options['handle_factory'] ?? new \FluentSmtpLib\GuzzleHttp\Handler\CurlFactory(50);
        if (isset($options['select_timeout'])) {
            $this->selectTimeout = $options['select_timeout'];
        } elseif ($selectTimeout = \FluentSmtpLib\GuzzleHttp\Utils::getenv('GUZZLE_CURL_SELECT_TIMEOUT')) {
    ...
    public function __invoke(\FluentSmtpLib\Psr\Http\Message\RequestInterface $request, array $options) : \FluentSmtpLib\GuzzleHttp\Promise\PromiseInterface
    {
        ...
        $id = (int) $easy->handle;
        $promise = new \FluentSmtpLib\GuzzleHttp\Promise\Promise([$this, 'execute'], function () use($id) {
    ...
    public function tick() : void
    {
        ...
        if ($this->delays) {
            $currentTime = \FluentSmtpLib\GuzzleHttp\Utils::currentTime();
        ...
        \FluentSmtpLib\GuzzleHttp\Promise\Utils::queue()->add(\Closure::fromCallable([$this, 'tickInQueue']));
    ...
    private function addRequest(array $entry) : void
    {
        ...
        if (empty($easy->options['delay'])) {
            ...
            $this->delays[$id] = \FluentSmtpLib\GuzzleHttp\Utils::currentTime() + $easy->options['delay'] / 1000;
        }
    }
    /**
    ...
    private function processMessages() : void
    {
        while ($done = \curl_multi_info_read($this->_mh)) {
            ...
            $entry['deferred']->resolve(\FluentSmtpLib\GuzzleHttp\Handler\CurlFactory::finish($this, $entry['easy'], $this->factory));
        }
    }
