 ...
 * @final
 */
class EachPromise implements \FluentSmtpLib\GuzzleHttp\Promise\PromisorInterface
{
    private $pending = [];
    ...
    {
        $this->iterable = \FluentSmtpLib\GuzzleHttp\Promise\Create::iterFor($iterable);
    ...
    {
        $this->mutex = \false;
        $this->aggregate = new \FluentSmtpLib\GuzzleHttp\Promise\Promise(function () : void {
            ...
            while ($promise = \current($this->pending)) {
                \next($this->pending);
                $promise->wait();
                if (\FluentSmtpLib\GuzzleHttp\Promise\Is::settled($this->aggregate)) {
                    return;
                }
            }
        });
    ...
    private function addPending() : bool
    {
        ...
        $promise = \FluentSmtpLib\GuzzleHttp\Promise\Create::promiseFor($this->iterable->current());
