...
use DateTime;
use FluentSmtpLib\Firebase\JWT\ExpiredException;
...
use FluentSmtpLib\SimpleJWT\InvalidTokenException;
use FluentSmtpLib\SimpleJWT\JWT as SimpleJWT;
use FluentSmtpLib\SimpleJWT\Keys\KeyFactory;
use FluentSmtpLib\SimpleJWT\Keys\KeySet;
use TypeError;
...
class AccessToken
{
     ...
     */
    public function __construct(?callable $httpHandler = null, ?\FluentSmtpLib\Psr\Cache\CacheItemPoolInterface $cache = null)
    {
        $this->httpHandler = $httpHandler ?: \FluentSmtpLib\Google\Auth\HttpHandler\HttpHandlerFactory::build(\FluentSmtpLib\Google\Auth\HttpHandler\HttpClientCache::getHttpClient());
        $this->cache = $cache ?: new \FluentSmtpLib\Google\Auth\Cache\MemoryCacheItemPool();
    }
    /**
    ...
    public function verify($token, array $options = [])
    {
        ...
        try {
            ...
            // firebase/php-jwt 5+
        } catch (\FluentSmtpLib\SimpleJWT\InvalidTokenException $e) {
            // simplejwt
    ...
    {
        $this->checkSimpleJwt();
        $jwkset = new \FluentSmtpLib\SimpleJWT\Keys\KeySet();
        foreach ($certs as $cert) {
            $jwkset->add(\FluentSmtpLib\SimpleJWT\Keys\KeyFactory::create($cert, 'php'));
        }
    ...
    private function verifyRs256($token, array $certs, $audience = null, $issuer = null)
    {
        ...
        foreach ($certs as $cert) {
            ...
            $keys[$cert['kid']] = new \FluentSmtpLib\Firebase\JWT\Key($publicKey, 'RS256');
        }
    ...
    public function revoke($token, array $options = [])
    {
        ...
        $request = new \FluentSmtpLib\GuzzleHttp\Psr7\Request('POST', self::OAUTH2_REVOKE_URI, ['Cache-Control' => 'no-store', 'Content-Type' => 'application/x-www-form-urlencoded'], $body);
    ...
    private function retrieveCertsFromLocation($url, array $options = [])
    {
        ...
        $httpHandler = $this->httpHandler;
        $response = $httpHandler(new \FluentSmtpLib\GuzzleHttp\Psr7\Request('GET', $url), $options);
    ...
    {
        if (!\class_exists(\FluentSmtpLib\phpseclib3\Crypt\RSA::class)) {
    ...
    {
        $key = \FluentSmtpLib\phpseclib3\Crypt\PublicKeyLoader::load(['n' => new \FluentSmtpLib\phpseclib3\Math\BigInteger($this->callJwtStatic('urlsafeB64Decode', [$modulus]), 256), 'e' => new \FluentSmtpLib\phpseclib3\Math\BigInteger($this->callJwtStatic('urlsafeB64Decode', [$exponent]), 256)]);
    ...
    private function checkSimpleJwt()
    {
        ...
        if (!\class_exists(\FluentSmtpLib\SimpleJWT\JWT::class)) {
    ...
    {
        return \call_user_func_array([\FluentSmtpLib\Firebase\JWT\JWT::class, $method], $args);
        // @phpstan-ignore-line
    }
    /**
     * Provide a hook to mock calls to the JWT static methods.
    ...
    {
        return \call_user_func_array([\FluentSmtpLib\SimpleJWT\JWT::class, 'decode'], $args);
    }
    /**
