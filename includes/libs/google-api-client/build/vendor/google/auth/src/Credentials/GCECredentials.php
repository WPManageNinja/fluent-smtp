...
use FluentSmtpLib\Google\Auth\Iam;
use FluentSmtpLib\Google\Auth\IamSignerTrait;
use FluentSmtpLib\Google\Auth\ProjectIdProviderInterface;
use FluentSmtpLib\Google\Auth\SignBlobInterface;
use FluentSmtpLib\GuzzleHttp\Exception\ClientException;
use FluentSmtpLib\GuzzleHttp\Exception\ConnectException;
use FluentSmtpLib\GuzzleHttp\Exception\RequestException;
use FluentSmtpLib\GuzzleHttp\Exception\ServerException;
 ...
 */
class GCECredentials extends \FluentSmtpLib\Google\Auth\CredentialsLoader implements \FluentSmtpLib\Google\Auth\SignBlobInterface, \FluentSmtpLib\Google\Auth\ProjectIdProviderInterface, \FluentSmtpLib\Google\Auth\GetQuotaProjectInterface
{
    use IamSignerTrait;
    // phpcs:disable
     ...
     */
    public function __construct(?\FluentSmtpLib\Google\Auth\Iam $iam = null, $scope = null, $targetAudience = null, $quotaProject = null, $serviceAccountIdentity = null, ?string $universeDomain = null)
    {
        $this->iam = $iam;
    ...
    {
        $httpHandler = $httpHandler ?: \FluentSmtpLib\Google\Auth\HttpHandler\HttpHandlerFactory::build(\FluentSmtpLib\Google\Auth\HttpHandler\HttpClientCache::getHttpClient());
        ...
        for ($i = 1; $i <= self::MAX_COMPUTE_PING_TRIES; $i++) {
            try {
                ...
                // the metadata resolution was particularly slow. The latter case is
                // "unlikely".
                $resp = $httpHandler(new \FluentSmtpLib\GuzzleHttp\Psr7\Request('GET', $checkUri, [self::FLAVOR_HEADER => 'Google']), ['timeout' => self::COMPUTE_PING_CONNECTION_TIMEOUT_S]);
                return $resp->getHeaderLine(self::FLAVOR_HEADER) == 'Google';
            } catch (\FluentSmtpLib\GuzzleHttp\Exception\ClientException $e) {
            } catch (\FluentSmtpLib\GuzzleHttp\Exception\ServerException $e) {
            } catch (\FluentSmtpLib\GuzzleHttp\Exception\RequestException $e) {
            } catch (\FluentSmtpLib\GuzzleHttp\Exception\ConnectException $e) {
            }
        }
        if (\PHP_OS === 'Windows') {
    ...
    {
        $resp = $httpHandler(new \FluentSmtpLib\GuzzleHttp\Psr7\Request('GET', $uri, [self::FLAVOR_HEADER => 'Google']));
