...
use FluentSmtpLib\Google\Auth\Credentials\ExternalAccountCredentials;
use FluentSmtpLib\Google\Auth\Credentials\ImpersonatedServiceAccountCredentials;
use FluentSmtpLib\Google\Auth\Credentials\InsecureCredentials;
 ...
 */
abstract class CredentialsLoader implements \FluentSmtpLib\Google\Auth\GetUniverseDomainInterface, \FluentSmtpLib\Google\Auth\FetchAuthTokenInterface, \FluentSmtpLib\Google\Auth\UpdateMetadataInterface
{
    ...
    public static function makeCredentials($scope, array $jsonKey, $defaultScope = null)
    {
        ...
        if ($jsonKey['type'] == 'service_account') {
            ...
            return new \FluentSmtpLib\Google\Auth\Credentials\ServiceAccountCredentials($scope, $jsonKey);
        }
        if ($jsonKey['type'] == 'authorized_user') {
            $anyScope = $scope ?: $defaultScope;
            return new \FluentSmtpLib\Google\Auth\Credentials\UserRefreshCredentials($anyScope, $jsonKey);
        }
        if ($jsonKey['type'] == 'impersonated_service_account') {
            $anyScope = $scope ?: $defaultScope;
            return new \FluentSmtpLib\Google\Auth\Credentials\ImpersonatedServiceAccountCredentials($anyScope, $jsonKey);
        }
        if ($jsonKey['type'] == 'external_account') {
            $anyScope = $scope ?: $defaultScope;
            return new \FluentSmtpLib\Google\Auth\Credentials\ExternalAccountCredentials($anyScope, $jsonKey);
        }
     ...
     */
    public static function makeHttpClient(\FluentSmtpLib\Google\Auth\FetchAuthTokenInterface $fetcher, array $httpClientOptions = [], ?callable $httpHandler = null, ?callable $tokenCallback = null)
    {
        $middleware = new \FluentSmtpLib\Google\Auth\Middleware\AuthTokenMiddleware($fetcher, $httpHandler, $tokenCallback);
        $stack = \FluentSmtpLib\GuzzleHttp\HandlerStack::create();
        $stack->push($middleware);
        return new \FluentSmtpLib\GuzzleHttp\Client(['handler' => $stack, 'auth' => 'google_auth'] + $httpClientOptions);
    }
    /**
    ...
    {
        return new \FluentSmtpLib\Google\Auth\Credentials\InsecureCredentials();
    }
    /**
