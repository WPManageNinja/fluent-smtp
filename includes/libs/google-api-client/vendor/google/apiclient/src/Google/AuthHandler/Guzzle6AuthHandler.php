...
use FluentMailLib\Google\Auth\FetchAuthTokenCache;
use FluentMailLib\Google\Auth\Middleware\AuthTokenMiddleware;
use FluentMailLib\Google\Auth\Middleware\ScopedAccessTokenMiddleware;
use FluentMailLib\Google\Auth\Middleware\SimpleMiddleware;
...
class Google_AuthHandler_Guzzle6AuthHandler
{
    ...
    {
        return new Client(['base_uri' => $http->getConfig('base_uri'), 'exceptions' => \true, 'verify' => $http->getConfig('verify'), 'proxy' => $http->getConfig('proxy')]);
    }
}
