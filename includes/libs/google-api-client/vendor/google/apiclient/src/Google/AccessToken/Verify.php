 ...
 */
use FluentMailLib\Firebase\JWT\ExpiredException as ExpiredExceptionV3;
use FluentMailLib\Firebase\JWT\SignatureInvalidException;
...
use FluentMailLib\Google\Auth\Cache\MemoryCacheItemPool;
use FluentMailLib\Stash\Driver\FileSystem;
use FluentMailLib\Stash\Pool;
/**
...
class Google_AccessToken_Verify
{
    ...
    {
        $jwtClass = 'JWT';
        if (\class_exists('FluentMailLib\\Firebase\\JWT\\JWT')) {
            $jwtClass = 'FluentMailLib\\Firebase\\JWT\\JWT';
        }
    ...
    {
        if (\class_exists('FluentMailLib\\phpseclib\\Crypt\\RSA')) {
            return 'FluentMailLib\\phpseclib\\Crypt\\RSA';
        }
        return 'Crypt_RSA';
    }
    private function getBigIntClass()
    {
        if (\class_exists('FluentMailLib\\phpseclib\\Math\\BigInteger')) {
            return 'FluentMailLib\\phpseclib\\Math\\BigInteger';
        }
    ...
    private function getOpenSslConstant()
    {
        ...
        if (\class_exists('FluentMailLib\\Crypt_RSA')) {
    ...
    private function setPhpsecConstants()
    {
        if (\filter_var(\getenv('GAE_VM'), \FILTER_VALIDATE_BOOLEAN)) {
            if (!\defined('FluentMailLib\\MATH_BIGINTEGER_OPENSSL_ENABLED')) {
                \define('FluentMailLib\\MATH_BIGINTEGER_OPENSSL_ENABLED', \true);
            }
            if (!\defined('FluentMailLib\\CRYPT_RSA_MODE')) {
                \define('FluentMailLib\\CRYPT_RSA_MODE', \constant($this->getOpenSslConstant()));
            }
        }
    }
}
