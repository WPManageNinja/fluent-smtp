...
class OAuth2 implements FetchAuthTokenInterface
{
    ...
    private function jwtDecode($idToken, $publicKey, $allowedAlgs)
    {
        if (\class_exists('FluentMailLib\\Firebase\\JWT\\JWT')) {
            return \FluentMailLib\Firebase\JWT\JWT::decode($idToken, $publicKey, $allowedAlgs);
        }
        return \FluentMailLib\JWT::decode($idToken, $publicKey, $allowedAlgs);
    }
    private function jwtEncode($assertion, $signingKey, $signingAlgorithm)
    {
        if (\class_exists('FluentMailLib\\Firebase\\JWT\\JWT')) {
            return \FluentMailLib\Firebase\JWT\JWT::encode($assertion, $signingKey, $signingAlgorithm);
        }
        return \FluentMailLib\JWT::encode($assertion, $signingKey, $signingAlgorithm);
    }
    /**
