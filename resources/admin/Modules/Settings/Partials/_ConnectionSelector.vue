<template>
    <div>
        <div v-if="connection.provider && !showAll">
            <div :class="'con_'+connection.provider" class="fsmtp_connection_selected">
            ...
            <el-button @click="showAll = !showAll" size="small" icon="FsmIconEdit" type="default">{{ $t('change') }}</el-button>
        </div>
        <el-radio-group @change="showAll = false" v-else class="fss_connections" v-model="connection.provider">
            <el-radio-button :class="'con_'+providerName" v-for="(provider, providerName) in providers" :key="providerName" :value="providerName">
